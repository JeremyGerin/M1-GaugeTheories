\chapter{Variétés différentielle}
\minisommairech
...
\section{Fibrés vectoriels et fibré principal}
...
\end{matrix}
$$
Enfin, on dispose d'une action lisse de $G$ sur $P$ définie pour $g\in G$ et $(\phi_\alpha(x), h)\in H_\alpha$ par 
$$g.(\phi_\alpha(x), h) = (\phi_\alpha(x), gh) \in H_\alpha$$
...
\be 
	...
	\item $G$ agit de façon lisse sur $P$ tel que chaque fibre $P_x = \pi^{-1}(x)$ soit stable par $G$ et tel que $G$ agisse transitivement sur chaque fibre $P_x$.
\ee
...
Notons $t_{\beta\alpha} = d\phi_{\beta\alpha} \circ \phi_\alpha : U_\alpha \cap U_\beta \to \GL(\R^m)$ le cocycle canoniquement donné par l'atlas de $\mathcal{M}$ et soient les cocycles
$$t^*_{\beta\alpha} = {}^tt_{\beta\alpha}^{-1} : U_\alpha \cap U_\beta \to \GL((\R^m)^*)$$
$$\otimes^r t_{\beta\alpha} \otimes^s t^*_{\beta\alpha} : U_\alpha \cap U_\beta \to \GL(\otimes^r (\R^m) \otimes^s (\R^m)^*)$$

\begin{defi}[Fibré tangent]
Le fibré tangent de $\mathcal{M}$ noté $\tau_\mathcal{M} : T\mathcal{M} \to \mathcal{M}$ est donné par le cocycle $t_{\beta\alpha}$. \\
On appelle champs de vecteur une section du fibré $\tau_\mathcal{M}$.
\end{defi}

\begin{defi}[Fibré cotangent]
Le fibré cotangent de $\mathcal{M}$ noté $\tau^*_\mathcal{M} : T^*\mathcal{M} \to \mathcal{M}$ est donné par le cocycle $t^*_{\beta\alpha}$. 
\end{defi}

\begin{defi}[Fibré tensoriel]
Soient $r,s\in \N$, alors le fibré tensoriel d'ordre $(r,s)$ ($r$ contravariant et $s$ covariant) de $\mathcal{M}$, noté $\tau^{(r,s)}_\mathcal{M} : T^{(r,s)}\mathcal{M} \to \mathcal{M}$ est donné par le cocycle $\otimes^r t_{\beta\alpha} \otimes^s t^*_{\beta\alpha}$. \\
...
\ee 
Ou de façon équivalente, c'est la donnée d'une 1-forme différentielle sur $\mathcal{M}$ à valeur dans $\mathfrak{g}$, $\omega \in  \Omega^1(P, \mathfrak{g})$ telle que
...
\subsubsection{Connexion de Koszul:}

\begin{defi}[Connexion de Koszul]
Soit $E \to \mathcal{M}$ un fibré vectoriel. Une connexion de Koszul est une application $\nabla: \Gamma(T\mathcal{M}) \times \Gamma(E) \to \Gamma(E)$ telle que pour $f \in \mathscr{C}^\infty(\mathcal{M})$, $X\in \Gamma(T\mathcal{M})$ et $\sigma \in \Gamma(E)$ on a:
\be
	\item $\nabla$ est $\R$-bilinéaire
	\item $\nabla_{fX}\sigma = f \nabla_X\sigma$
	\item $\nabla_X(f\sigma) = f\nabla_X\sigma + X(f)\sigma$
\ee
\end{defi}

Soit $\pi : P \to \mathcal{M}$ un fibré principal de groupe structural $G$, soit $\rho : G \to \GL(V)$ une représentation de $G$ et soit $p \in P \mapsto \Hor(T_pP)$ une connexion d'Ehresmann sur $P$. Notons $\tilde{\pi} : E = P\times_\rho V \to \mathcal{M}$ et $s : P\times V \to E$ la surjection canonique. Soit $(p, v) \in P\times V$ un représentant de $y\in E$ et posons 
$$H_y = T_{(p,v)}s(\Hor(T_pP) \times \{v\})$$ 
Cette définition ne dépend pas du choix du représentant $(p, v)$ de $y$ puisque en notant $\tilde{R_g}: (p,v) \mapsto (p, v).g$, on a $s \circ \tilde{R_g} = s$ donc $T_{(p,v).g} s \circ T_{(p,v)} \tilde{R_g} = T_{(p, v)} s$ et
$$T_{(p,v)}\tilde{R_g}(\Hor(T_pP)\times \{v\}) = T_pR_g(\Hor(T_pP)) \times \{\rho(g^{-1})v\} = \Hor(T_{p.g}P) \times \{\rho(g^{-1})v\}$$

Soit $Y\in T_yE$ un vecteur tangent à $E$ en $y$. Notons $h: T_yE \to H_y$ la projection sur $H_y$ parallèlement à $T_y(E_{\tilde{\pi(y)}})$. Comme la fibre $E_{\tilde{\pi(y)}}$ est un espace vectoriel on identifie canoniquement $E_{\tilde{\pi(y)}}$ et $T_yE_{\tilde{\pi(y)}}$, et on note $v(Y) = Y - h(Y)\in E_{\tilde{\pi(y)}}$. \\

On pose alors, pour tout champs de vecteur $X$ sur $\mathcal{M}$, pour toute section $\sigma \in \Gamma(E)$ et tout $x\in \mathcal{M}$ 
$$\nabla_X\sigma(x) = v(T_x\sigma(X(x)))$$
Alors, $\nabla: \Gamma(T\mathcal{M})\times \Gamma(E) \to \Gamma(E)$ est une connexion de Koszul.

\begin{pro}
Soit $\nabla$ une connexion de Koszul sur $T\mathcal{M}$. Alors $\nabla$ se prolonge de façon unique en une connexion de Koszul sur chaque $T^{(r,s)}\mathcal{M}$ qu'on note encore $\nabla$ telle que:
\be
	\item $\nabla_X = \mathscr{L}_X$ sur $T^{(0,0)}\mathcal{M} = \mathscr{C}^\infty(\mathcal{M})$
	\item $\forall \sigma\in \Gamma(T^{(r,s)}\mathcal{M}), \quad\sigma' \in \Gamma(T^{(r', s')}\mathcal{M}), \quad X \in \Gamma(T\mathcal{M})$ on a 
	$$\nabla_X(\sigma \otimes \sigma') = \nabla_X(\sigma)\otimes \sigma' + \sigma \otimes \nabla_X(\sigma')$$
	\item commute avec les contractions
\ee
\end{pro}