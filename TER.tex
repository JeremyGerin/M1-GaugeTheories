...
\begin{document}
	...
	\mainmatter
		...
		Alors les applications $\gamma_{\beta\alpha}$ définissent une relation d'équivalence sur l'union disjointe $\DUnion_{\alpha \in A}\phi_\alpha(U_\alpha)\times G$. Notons $P$ l'espace quotient qui est une variété lisse. \\

		On dispose aussi d'une application lisse $\pi : P \to \mathcal{M}$ telle qu'il existe des difféomorphismes $\psi_\alpha : \pi^{-1}(U_\alpha) \to \phi_\alpha(U_\alpha) \times G$ tels que $\pi_\alpha \circ \psi_\alpha = \pi \circ \phi_\alpha$ appelés trivialisation locales où $\pi_\alpha : \phi_\alpha(U_\alpha) \times G \to \phi_\alpha(U_\alpha)$ désigne la projection par rapport à la première composante. Aussi on a $\psi_\beta \circ \psi_\alpha^{-1} = \gamma_{\beta\alpha}$. \\

		Pour chaque $x\in \mathcal{M}$ on définit la fibre au dessus de $x$ comme $P_x = \pi^{-1}(x)$. De plus, G agit à droite de façon lisse sur $P$ tel que chaque $\pi^{-1}(U_\alpha)$ est stable par $G$ et tel que chaque trivalisation locale $\psi_\alpha$ vérifie $\phi_\alpha(x.g) = \phi_\alpha(x).g$ pour tout $x\in \pi^{-1}(U_\alpha)$ et tout $g\in G$ et où l'action à droite sur $\phi_\alpha(U_\alpha)\times G$ est la multiplication à droite sur la seconde composante.
